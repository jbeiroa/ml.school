...
class Backend(ABC):
    ...
    @abstractmethod
    def save(self, model_input: pd.DataFrame, model_output: list, metrics: dict | None = None) -> None:
        ...
        Args:
            ...
            metrics: The prediction metrics to log.
...
class Local(Backend):
    ...
    def save(self,
             ...
             model_output: list, output_type: str = "sqlite",
             output_path: str | None = None,
             metrics: dict | None = None) -> None:
        """Save production data to a SQLite database, jsonl or csv file.

        If the database doesn't exist, this function will create it.
        """
        self._info(f"Storing production data in {output_type}...")
        ...
        data["target"] = None
        data["latency_ms"] = metrics.get("latency_total_ms") if metrics else None
        data["error"] = metrics.get("error") if metrics else None
        ...
        if output_type == "sqlite":
            ...
            if output_path is None:
                self._error("Output path must be provided when output type is jsonl.")
                return

            data.to_json(output_path, orient="records", lines=True)
        elif output_type == "csv":
            if output_path is None:
                self._error("Output path must be provided when output type is csv.")
                return

            data.to_csv(output_path, index=False)
        else:
            self._error(f"Unsupported output type: {output_type}")
            return
